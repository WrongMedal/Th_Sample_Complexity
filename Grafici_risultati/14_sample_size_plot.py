...
def plot_exp_results(rete, exp):
    ...
    print(f"📂 Caricamento file: {full_path}")

    try:
        df = pd.read_csv(full_path)
    except FileNotFoundError:
        print("❌ File non trovato. Controlla che il percorso e i nomi siano corretti.")
        return
    ...
    # Rinomina colonne multi-index
    grouped.columns = ['layer_structure', 'sample_size',
    ...
    palette = sns.color_palette("Set1", len(layer_structures))
    ...
    for i, ls in enumerate(layer_structures):
        ...
        # Linea media
        axes[0].plot(data['sample_size'], data['test_acc_mean'],
                     label=f"{ls}", marker='o', color=color)

        # Upper bound (media + std)
        axes[0].plot(data['sample_size'],
                     data['test_acc_mean'] + data['test_acc_std'],
                     linestyle='--', color=color, alpha=0.5)

        # Lower bound (media - std)
        axes[0].plot(data['sample_size'],
                     data['test_acc_mean'] - data['test_acc_std'],
                     linestyle='--', color=color, alpha=0.5)
    ...
    for i, ls in enumerate(layer_structures):
        ...
        # Linea media
        axes[1].plot(data['sample_size'], data['test_loss_mean'],
                     label=f"{ls}", marker='o', color=color)

        # Upper bound
        axes[1].plot(data['sample_size'],
    ...
    # 💾 Salvataggio prima di show()
    save_path = os.path.join(output_folder, f"Exp{exp}_Rete{rete.upper()}_Accuracy_Loss.png")
    plt.savefig(save_path, dpi=300)
    print(f"✅ Grafico combinato salvato in: {save_path}")

    plt.show()

#@title Esempio
plot_exp_results(rete='A', exp=1)